...
> 目标平台: Windows (25H2, build 26200), 125% DPI 缩放 (scaleFactor=1.25), 单显示器 2560x1600
> 状态: 修复完成, 待作者合并
...
- 机器 scaleFactor=1.25: `e.screenX` 是**物理像素**, `setPosition` 用 **DIP(逻辑像素)**
- 效果: 窗口移动 = 鼠标移动 × 1.25, 越拖越偏右下
- 证据: `probe-units.ps1` 输出 `scaleFactor=1.25`, `probe-position.ps1` 验证 `setPosition(100)` → `getBounds` 差 100(确认 setPosition 用 DIP)
...
- 混合 DPI 多显示器: 绝对 DIP 位移在单显示器 125% 下正确, 跨异缩放显示器未实测
